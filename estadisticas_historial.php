...
if (empty($_SESSION['plantel_id'])) {
    header('Location: ucol-srvc-coord.php');
    exit;
}
...
try {
    ...
    if ($club_id) {
        ...
        if ($club_sel) {
            ...
            if (!empty($club_sel['parcial1_fin']) && !empty($club_sel['parcial2_fin'])) {
                $p1 = $club_sel['parcial1_fin'];
                $p2 = $club_sel['parcial2_fin'];
                $rangos = [
                    0 => [$club_sel['fecha_inicio'],                       $club_sel['fecha_fin']],
                    1 => [$club_sel['fecha_inicio'],                       $p1],
                    2 => [date('Y-m-d', strtotime($p1) + 86400),           $p2],
                    3 => [date('Y-m-d', strtotime($p2) + 86400),           $club_sel['fecha_fin']],
                ];
            } else {
                $dur = max(86400, $ff - $fi);
                $t   = intdiv($dur, 3);
...
<head>
    ...
    <style>
        ...
        *,*::before,*::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        .sb-hist-link { display:inline-flex; align-items:center; gap:.3rem; font-size:.66rem; font-weight:600;
                        color:var(--accent); text-decoration:none; padding:.25rem .6rem; border:1px solid var(--gray-200);
                        border-radius:20px; white-space:nowrap; transition:all .2s; flex-shrink:0; }
        .sb-hist-link:hover { background:var(--accent); color:#fff; border-color:var(--accent); }
...
<aside class="sidebar">
    <div class="sb-head">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:.5rem;margin-bottom:.5rem">
            <div class="sb-title" style="margin-bottom:0">
    ...
    <div class="dh-top">
        <div>
            ...
            <div class="dh-chips">
                ...
                <span class="chip"><?= ucfirst($club_sel['semestre']) ?> <?= $club_sel['anio'] ?></span>
            </div>
        </div>
